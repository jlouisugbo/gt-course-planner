...
export function Semesters({ semester, courses, containerId }: SemestersProps) {
    ...
    return (
        <Droppable id={containerId}>
            <div
                className={`${getSemesterColorClass(
                    semester,
                    hasItems
                )} border-2 border-dashed rounded-lg p-4 min-h-[140px] transition-all duration-200 ${
                    hasItems ? "shadow-lg" : "hover:bg-gray-100"
                }`}
            >
                <div className="flex items-center gap-2 mb-3">
                    <span className="text-lg">{getSemesterIcon(semester)}</span>
                    <h4
                        className={`font-medium ${getSemesterHeaderClass(
                            semester,
                            hasItems
                        )}`}
                    >
