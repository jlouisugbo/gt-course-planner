...
const CourseDetailsModal: React.FC<CourseDetailsModalProps> = ({
  ...
  const { missingPrereqs, warnings, isBlocked } = validation;
  ...
  const getWorkloadColor = (workload: number) => {
    if (workload <= 8) return 'text-green-600';
    if (workload <= 12) return 'text-yellow-600';
    if (workload <= 16) return 'text-orange-600';
    return 'text-red-600';
  };

  // Get completed and planned courses for prerequisite checking
          ...
          {isBlocked && (
            <Alert className="border-red-200 bg-red-50">
              ...
              <AlertDescription className="text-red-800">
                <strong>Cannot add course:</strong> You must complete the following prerequisites first.
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card>
              <CardContent className="p-4 text-center">
                ...
                <div className={cn("text-lg font-bold", getDifficultyColor(course.difficulty))}>
                  {course.difficulty}/5
                </div>
            ...
            <Card>
              <CardContent className="p-4 text-center">
                ...
                <div className={cn("text-lg font-bold", getWorkloadColor(course.workload))}>
                  {course.workload}h
                </div>
            ...
            <Card>
              <CardContent className="p-4 text-center">
                ...
                <div className="text-lg font-bold text-slate-900">
                  {[course.offerings.fall && 'F', course.offerings.spring && 'S', course.offerings.summer && 'Su'].filter(Boolean).join('/')}
                </div>
                    ...
                    {prereq.type === 'course' && prereq.courses && (
                      <div>
                        <p className="text-sm text-slate-600 mb-2">
                          Required courses {prereq.logic === 'OR' ? '(any one of)' : '(all required)'}:
                        </p>
                        <div className="flex flex-wrap gap-2">
                          {prereq.courses.map((courseCode) => {
                            const isCompleted = completedCourses.some(c => c.code === courseCode);
                            const isPlanned = plannedCourses.some(c => c.code === courseCode);
                            ...
                            return (
                              <Badge
                                ...
                                className={cn(
                                  ...
                                  isMissing ? "bg-red-100 text-red-800 border-red-300" :
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {course.attributes.map((attribute) => (
                  <Badge key={attribute} variant="secondary" className="border-slate-300">
                    {attribute}
                  </Badge>
                ))}
              </div>
